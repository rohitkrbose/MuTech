...
def multivariate_Gaussian (x, MU, COV):
	return (math.exp(-0.5*np.dot((np.dot(np.transpose(x-MU),np.linalg.inv(COV))),(x-MU))))/math.sqrt(np.linalg.det(2*math.pi*COV))
...
def getChromaBeats ():
	x, fs = librosa.load(sys.argv[1], offset=int(sys.argv[2]), duration=int(sys.argv[3])-int(sys.argv[2]))
	window_size = 4420
	hop_size = 2210
	...
	for i in range (1,b):
		temp = np.mean(x_chroma[:,beats[i-1]:beats[i]],axis=1)
		x_beat_chroma = np.vstack((x_beat_chroma,temp))
	temp = np.mean(x_chroma[:,beats[i-1]:T],axis=1)
	# x_beat_chroma = np.vstack((x_beat_chroma,temp))
	return (np.transpose(x_beat_chroma),beats)
...
def Viterbi ():
	...
	for i in range (1,T):
		for j in range (0,K):
			m = 0
			for k in range (0,K):
				if (T1[k][i-1]*A[k][j] > T1[m][i-1]*A[m][j]):
					m = k 
			T1[j][i] = B[j][i] * T1[m][i-1]*A[m][j]
			T2[j][i] = m
	z = np.zeros(T)
	X = [None] * T
